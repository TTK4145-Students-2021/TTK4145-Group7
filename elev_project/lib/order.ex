defmodule Order do
  ...
  def send_order(order, from) do
    ...
    {node_costs, bad_nodes_cost_calc} =
      GenServer.multi_call(
        [node() | Node.list()],
        @name,
        {:calc_cost, {elevator_number, floor, order_type}},
        @multi_call_timeout
      )
    ...
    {winning_elevator, cost} =
      if from === :order_watchdog do
        ...
        |> Enum.map(fn x ->
          ...
          cost =
            if elev_n === order_elevator_number do
    ...
    {acks, bad_nodes_new_order} =
      GenServer.multi_call(
        Node.list(),
        @name,
        {:new_order, {winning_elevator, floor, order_type}},
        @multi_call_timeout
      )
    ...
    if length(bad_nodes_cost_calc) > 0 or length(bad_nodes_new_order) > 0 do
      Logger.warning(%{
        bad_nodes_cost_calc: bad_nodes_cost_calc,
  ...
  def order_completed(floor) do
    ...
    GenServer.multi_call(
      [node() | Node.list()],
      @name,
      {:order_completed, {elev_num, floor, :dummy}},
      @multi_call_timeout
    )
  end
  ...
  def compare_order_states() do
    {good_nodes, bad_nodes} =
      GenServer.multi_call([node() | Node.list()], @name, :get_order_state, @multi_call_timeout)

    all_available_order_maps =
      Enum.reduce(Keyword.values(good_nodes), [], fn x, acc -> acc ++ [elem(x, 1)] end)

    all_orders =
      Enum.reduce(all_available_order_maps, %{}, fn order_map, combined_orders ->
  ...
  @impl true
  def handle_call(
        {:calc_cost, {elevator_that_sent_order, ordered_floor, :cab}},
        _from,
    ...
    cost =
      if(elevator_number === elevator_that_sent_order) do
        calculate_cost(
          {elevator_number, ordered_floor, :cab},
          order_map,
  ...
  def handle_call({:calc_cost, order}, _from, {elevator_number, order_map}) do
    ...
    cost =
      calculate_cost(
        {elevator_number, ordered_floor, order_type},
        order_map,
        Elevator.get_elevator_state()
      )
  ...
  defp get_active_orders(
         order_map,
         elevator_number,
         direction,
         filter,
         floor_range \\ 0..@top_floor
       ) do
  ...
  defp calculate_cost(order, order_map, elevator_state) do
    ...
    travel_distance =
      abs(elevator_current_floor - checking_floor) + abs(checking_floor - ordered_floor)
  ...
  defp create_order_map(num_of_elevators, top_floor, order_map \\ %{}) do
    order_map =
      Enum.reduce(ButtonPoller.Supervisor.get_all_buttons(top_floor), order_map, fn x,
                                                                                    order_map ->
